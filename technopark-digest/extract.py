...
# Ключевое слово пишем через [Тт], а не через IGNORECASE: флаг ослабил бы и
# класс [А-ЯЁ] для самого названия, и в него полез бы строчный мусор.
PARK_PATTERN_RU = re.compile(
    r"(?:[Тт]ехнопарк[а-я]*|[Тт]ехнополис[а-я]*|ОЭЗ|[Оо]соб[а-я]+\s+экономическ[а-я]+\s+зон[а-я]+)"
    r"\s+[«\"]?([А-ЯЁA-Z][\wА-Яа-яЁё\-]*(?:\s+[А-ЯЁа-яё][\wА-Яа-яЁё\-]*){0,2})[»\"]?"
)

# Русские слова-действия: всё от них правее названием площадки не является.
RU_ACTION_WORDS = {
    "внедряет", "внедрил", "развивает", "развил", "открыл", "открыли",
    "открылся", "открылась", "запустил", "запустили", "построит",
    "построил", "получил", "получили", "создан", "создана", "создали",
    "стал", "стала", "планирует", "планируют", "привлёк", "привлек",
    "привлекли", "расширяет", "расширят", "начал", "начали", "введён",
    "введен", "ввели", "станет", "будет", "может", "готов", "готовит",
    "подписал", "представил", "показал", "принял", "вошёл", "вошел",
...
def trim_ru(phrase):
    """Обрезаем русское название по первому слову-действию."""
    kept = []
    for word in phrase.split():
        if word.lower().strip(",.;:") in RU_ACTION_WORDS:
            break
        kept.append(word)
    return " ".join(kept)
...
def extract_park(title, summary):
    ...
    if match:
        candidate = trim_ru(match.group(1).strip(" -—–"))
        if len(candidate) > 2:
            return candidate
    return ""
